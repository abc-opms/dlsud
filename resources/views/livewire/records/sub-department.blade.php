<div>
    <div class="mt-4 card p-3">
        ...
        <div class="row d-flex justify-content-between mb-3">
            ...
            <div class="col-sm-6 d-flex justify-content-end">
                ...
                <select class="form-select filter-record mt-1" wire:model="filterby">
                    <option selected value="subdept_code">Sub Dept Code</option>
                    <option value="description">Description</option>
                    <option selected value="dept_code">Sub Dept Code</option>
                </select>
            </div>
        </div>
        <div class="overflow-auto">
            <table class="table record-table">
                ...
                <tbody>
                    @if(!empty($subdept))
                    @for($i =0; $i <= (count($subdept)-1); $i++) <tr class="record" wire:click="show({{$subdept[$i]->id}},{{$i}})">
                        <td>{{$subdept[$i]->subdept_code}}</td>
                        <td>{{$subdept[$i]->description}}</td>
                        <td>{{$subdept[$i]->dept_code}}</td>
                        </tr>
                        @endfor
                        @endif
                </tbody>
    ...
    <!--  Modal -->
    <div wire:ignore.self class="modal fade" id="viewSubDepartment" tabindex="-1">
                ...
                <div class="modal-body">
                    <div class="row pe-3 ps-3 pb-3">
                        ...
                        <div class="col-md-6 mt-3">
                            <x-jet-label for="dept_code" value="SubDepartment Code" />
                            <div class="d-flex">
                                <input id="dept_code" class="r-field block mt-1 w-full @if(!Auth::user()->hasRole('Property')) record-f-disabled @endif" type="text" wire:model="subdept_code" />
                                @error('subdept_code')<span class="error text-danger">*</span> @enderror
                            </div>
                        </div>

                        <!---Sub dept_code-->
                        <div class="col-md-6 mt-3">
                            <x-jet-label for="fund_code" value="Department Code" />
                            <div class="d-flex">
                                <input id="fund_code" class="r-field block mt-1 w-full @if(!Auth::user()->hasRole('Property')) record-f-disabled @endif" type="text" wire:model="dept_code" />
