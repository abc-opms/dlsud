...
class ItemdSign extends Component
{
    ...
    {
        $this->endorsedto = Auth::user()->first_name . '' . Auth::user()->first_name
            . ' / ' . Carbon::now();
    ...
    {
        return [
            'endorsedto' => Auth::user()->school_id,
    ...
    {
        $subject = 'The RDF no. ' . $this->rdf_number . ' has been endorsed to Warehouse.';
        $form_type = "RDF";
        ...
        $linkp = "http://127.0.0.1:8000/p/itemdisposal/post/" . $this->rdf_number;
    ...
    {
        $type = "RDF";
        $message =  'The RDF no. ' . $this->rdf_number . ' has been endorsed to Warehouse.';
        ...
        $f = User::whereRoleIs('Property')->first();
        $linkf = "/p/itemdisposal/post/" . $this->rdf_number;
   ...
   Read at
    ...
    public function render()
    {
        ...
        $rdf = ItemDisposal::whereNotNull('approved_date')->whereNull('endorsed_date')->where('rdf_number', 'like', '%' . $this->search . '%')->latest()->get();

        return view('livewire.warehouse.itemd-sign', [
            'rdf' => $rdf,
        ]);
    }
}
