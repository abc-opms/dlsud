...
class RtfStatus extends Component
{
    ...
    {
        try {
            $c = TransferFurniture::where('from', Auth::user()->school_id)->where('rtf_number', $id)->first();
            if (!empty($c->rtf_number)) {
                $this->showdata($c->id);
            } else {
                return abort(404);
    ...
    public function render()
    {
        ...
        $rtf = TransferFurniture::where('rtf_number', 'like', '%' . $this->search . '%')->latest()->get();
        $rtf_cus = TransferFurniture::where('from', Auth::user()->school_id)->where('rtf_number', 'like', '%' . $this->search . '%')->latest()->get();
